  ...
  <div class="all">
    ...
    <div class="content">
      <div class="info" v-for="(item, index) in warnList" :key="index">
        <div class="name">
          <span v-if="item.status == 1" style="color:#00f8f8">[ 未处理 ]</span>
          <!-- <span v-if="item.status==2" style="color:#00f8f8">[ 未处理 ]</span> -->
          <span v-if="item.status == 3" style="color:#00f8f8"
            >[ 自动处理 ]</span
          >
          <span v-if="item.status == 4" style="color:#00f8f8"
            >[ 人工处理 ]</span
          >
          <span style="color:#e68600;margin-left:5px">{{ item.name }}</span>
        </div>
        <div class="cause">
          <div>
          ...
          <div style="padding-left:60px">
            ...
            <!-- <span v-if="item.status==2">负责人：自动处理</span> -->
            <span v-if="item.status == 3">负责人：自动处理</span>
        ...
        <div class="time">
          ...
          <div v-if="item.status != 1">
            <span v-if="item.status == 4" style="margin-right:35px"
              >响应时间：3</span
            >
            <span>持续时间：{{ item.durationTime }}</span>
          </div>
          <div v-else style="margin-top: -20px; margin-right:40px">
...
export default {
  ...
  methods: {
    // 获取告警列表
    getWarnList(startTime, endTime, status) {
      this.$axios({
        method: "GET",
    ...
    inquire() {
      ...
      // console.log(startTime, endTime, this.query.status);
      this.getWarnList(startTime, endTime, this.query.status);
    }
  },
  created() {
    // 获取告警列表
